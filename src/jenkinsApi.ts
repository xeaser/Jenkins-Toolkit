 ...
 */
export async function fetchBuildHistory(jenkinsUrl: string, jobName: string, username: string, apiToken: string, count: number = 5): Promise<JenkinsBuild[] | undefined> {
    // Using the /api/json endpoint with tree parameter to get limited build info
    const url = `${jenkinsUrl}/job/${jobName}/api/json?tree=builds[number,url,result,timestamp,duration]{,${count}}`;
